...
class TaskMonitor:
    ...
    def _check_task_status(self, task_id: str) -> bool:
        ...
        server_name = task_snapshot['server_name']
        monitor_type = task_snapshot.get('monitor_type', 'process')
        ...
        if monitor_type == 'process':
            return self._check_process_task(task_id, task_snapshot)
        # GPU空闲监控模式
        elif monitor_type == 'gpu_idle':
            return self._check_gpu_idle_task(task_id, task_snapshot)

        return True

    def _update_task_field(self, task_id: str, **updates) -> bool:
        """线程安全地更新任务字段"""
        with self.lock:
            if task_id not in self.tasks:
                return False
    ...
    def _check_process_task(self, task_id: str, task: Dict) -> bool:
        """检查进程监控任务"""
        ...
        if not result['success'] or not result['stdout'].strip():
            ...
            if task['notify_emails']:
                ...
                if not email_result['success']:
                    logger.warning(f"任务 {task_id} 邮件发送失败: {email_result['error']}")
    ...
    def _check_gpu_idle_task(self, task_id: str, task: Dict) -> bool:
        """
        检查GPU空闲监控任务
        ...
        logger.debug(f"[GPU空闲监听] 任务={task['task_name']}, GPU {gpu_id}, 显存={memory_used}MB, 空闲={is_idle}, 曾经忙碌={task.get('gpu_was_busy', False)}")

        # 首次检查：记录初始状态
        if not task.get('initial_check_done'):
            if not is_idle:
                # 初始状态是忙碌的，标记为已检测到忙碌状态
                self._update_task_field(task_id, initial_check_done=True, gpu_was_busy=True)
                logger.debug(f"[GPU空闲监听] GPU {gpu_id} 初始状态：忙碌（显存 {memory_used}MB）")
            else:
                # 初始状态就是空闲的
                self._update_task_field(task_id, initial_check_done=True)
                logger.debug(f"[GPU空闲监听] GPU {gpu_id} 初始状态：空闲（显存 {memory_used}MB）")
            return True

        # 如果GPU当前非空闲，标记已检测到忙碌状态
        if not is_idle:
            if not task.get('gpu_was_busy', False):
                logger.debug(f"[GPU空闲监听] GPU {gpu_id} 检测到忙碌状态（显存 {memory_used}MB）")
                self._update_task_field(task_id, gpu_was_busy=True)
            return True

        # 如果GPU当前空闲，但之前从未忙碌过
        if not task.get('gpu_was_busy', False):
            # GPU一直保持空闲，不发送通知（避免误报）
            logger.debug(f"[GPU空闲监听] GPU {gpu_id} 持续空闲，跳过通知")
            # 自动完成任务，状态标记为超时
            self._update_task_field(
                task_id,
                status='timeout',
        ...
        # GPU已从忙碌变为空闲，发送通知
        logger.info(f"[GPU空闲监听] GPU {gpu_id} 从忙碌变为空闲，发送通知")
        completed_at = datetime.now().isoformat()
        email_status = 'no_email'
